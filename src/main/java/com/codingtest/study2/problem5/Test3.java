...
public class Test3 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            String[] stringBoard = in.nextLine().split(" ");
            for (int j = 0; j < n; j++) {
                board[i][j] = Integer.parseInt(stringBoard[j]);
            }
        }
        ...
        in.nextLine();
        int[] moves = new int[m];
        String[] stringMoves = in.nextLine().split(" ");
        for (int i = 0; i < m; i++) {
            moves[i] = Integer.parseInt(stringMoves[i]);
        }

        System.out.println(new Test3().solution(n, board, moves));
    }
    ...
    private int solution(int n, int[][] board, int[] moves) {
        ...
        for (int move : moves) {
            for (int i = 0; i < n; i++) {
                ...
                if (target != 0) {
                    Integer prevStack = stack.isEmpty() ? 0 : stack.peek();
