...
public class Test5 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new Test5().solution(line));
    }
    ...
    private int solution(String line) {
        ...
        result = stickMap.size();

        for (Map.Entry<Integer, Integer> cutEntry : cutMap.entrySet()) {
            int cut = cutEntry.getKey();
            for (Map.Entry<Integer, Integer> stickEntry : stickMap.entrySet()) {
                int stickStart = stickEntry.getKey();
                int stickEnd = stickEntry.getValue();

                if (stickStart <= cut && stickEnd >= cut) {
                    result++;
                }
            }
        }
