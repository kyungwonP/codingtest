...
public class Test7576 {
    /**
     * 토마토 - BFS(너비 우선 탐색), 시간 O(N * M), 공간 O(N * M)
    ...
    private static int solution(Queue<Point> queue) {
        int result = -1;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    int nextRow = point.row + MOVE_ROW[j];
                    int nextCol = point.col + MOVE_COL[j];

                    if (nextRow >= 0 && nextRow < n
                            && nextCol >= 0 && nextCol < m
                            && tomatoes[nextRow][nextCol] == 0
                    ) {
