...
public class Test2630 {
    /**
     * 색종이 만들기 - 분할정복, 시간 O(n^2), 공간 O(n^2)
    ...
    private static void cut(int row, int col, int repeat) {
        if (isSameColor(row, col, repeat)) {
            if (board[row][col] == 0) {
                white++;
            } else {
                blue++;
        ...
        cut(row, col, diff);
        cut(row + diff, col, diff);
        cut(row, col + diff, diff);
        cut(row + diff, col + diff, diff);
    }

    private static boolean isSameColor(int row, int col, int repeat) {
