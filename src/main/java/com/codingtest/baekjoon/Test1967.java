...
public class Test1967 {
    ...
    private static int maxFirstNumber;
    private static int maxFirstCost;
    ...
    private static int solution() {
        ...
        visited[maxFirstNumber] = true;
        findMaxNode(maxFirstNumber, 0);

        return maxFirstCost;
    }

    private static void findMaxNode(int number, int sum) {
        List<Node> nodes = map.get(number);
        ...
        for (Node node : nodes) {
            if (!visited[node.number]) {
                ...
                findMaxNode(node.number, sum + node.cost);
            }
        }
