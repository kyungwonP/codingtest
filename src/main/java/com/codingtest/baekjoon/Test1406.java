...
public class Test1406 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (type == 'L' || type == 'D') {
                ...
                inputText(st.nextToken().charAt(0));
            } else if (type == 'D') {
                removeText();
            }
        }
    ...
    private static void inputText(char input) {
    ...
    private static void removeText() {
        ...
        result.delete(cursor, 1);
    }

    private static void moveCursor(char type) {
        if (type == 'L') {
            cursor = cursor - 1 < 0 ? 0 : cursor - 1;
        } else if (type == 'D') {
            cursor = cursor + 1 > result.length() ? result.length() : cursor + 1;
        }
    }
}
