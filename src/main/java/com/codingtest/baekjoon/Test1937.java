...
public class Test1937 {
    ...
    private static void dfs(int row, int col, List<Integer> trees) {
        result = Math.max(result, trees.size());

        int prevTreeCount = trees.get(trees.size() - 1);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (nextRow >= 0 && nextRow < n
                    && nextCol >= 0 && nextCol < n
                    && map[nextRow][nextCol] > prevTreeCount
            ) {
                trees.add(map[nextRow][nextCol]);
                dfs(nextRow, nextCol, trees);
